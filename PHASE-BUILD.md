# Task — Add Maxio subscription billing to eShopOnWeb (OpenAPI-spec variant)
...
> **Variant note:** this is the OpenAPI-spec edition of the task. It is identical to the
> plugin-based task except that all Maxio integration is driven from Maxio's **OpenAPI
> specification** rather than a Maxio SDK plugin. See *Maxio tooling* below.
...
## Sandbox entities (already seeded on site `apimatic-hackathon`)
...
## Run-harness constraints

*(Appended by the test-suite pipeline. Identical text for every arm, so it never changes the
A/B. Configured by `taskRulesFile` in `test-suite/config/suite.config.json` — set it to `""`
to stop appending this block.)*
