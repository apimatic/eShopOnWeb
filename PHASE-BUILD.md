...
- PayPal's **OpenAPI specifications** — located under the **`api-specs/paypal/`** folder —
  are the **authoritative contract** for **every** interaction with the API. Endpoints,
  path/query params, request and response schemas, auth schemes, server/base-URL templating,
  and error models all come from the specs. How you consume them — codegen a client or
  hand-write against them — is your call, as long as the specs are the contract you build to.
- You **may** consult official PayPal documentation as a **secondary** reference to clarify
  semantics or fill in behavior a spec describes ambiguously. The **spec is authoritative**:
  where a spec and any doc/web source conflict, the spec wins. Do not build against
  endpoints, fields, or shapes that don't appear in the spec.
- If a spec genuinely does not cover a capability you need — and official docs don't resolve
  it — **STOP and report the gap**. Do not invent endpoints/fields or work around the
  contract.
