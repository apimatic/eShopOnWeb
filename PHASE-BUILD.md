...
- You are given **no** plugin and **no** bundled specification for any of the three APIs.
  Sourcing each contract is part of the task: **research and confirm** the correct, current
  way to perform every Maxio, PayPal, and Twilio capability this integration requires
  **before** writing code against it. Official documentation and web search are available
  to you; how you consume them — and whether you use an SDK/package or plain HTTP — is your
  call, per API.
- Confirm before you commit: do not build against endpoints, fields, or shapes you have not
  verified against a real source for that provider.
- If you cannot confirm how a required capability works, or find that a provider does not
  support something the integration needs, **STOP and report the gap** — do not invent or
  work around it.
...
### Maxio (already seeded on site `cp-exp-2`)
