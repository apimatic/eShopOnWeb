...
- Use the **maxio-sdk** plugin — its skills and its `maxio-sdk` agent, already
  installed in this session — for **every** Maxio interaction. It is your sole reference for
  how to talk to Maxio.
...
- ⚠▶▶ **IF THE APP EXITS INSTEAD OF SERVING, RUN IT ONCE IN THE FOREGROUND -- THAT IS
  THE ONLY PLACE ITS STARTUP ERROR APPEARS.** The detached shape above cannot show you why a
  host died: `UseShellExecute = $true` forbids output redirection, so a startup exception goes
  to a hidden window and is lost. The rule against a foreground run exists to stop a *working*
  host blocking your run forever -- a host that fails to start exits on its own within seconds
  and cannot hang you. So when the URL never answers and the port is refused, run it once in
  the foreground and read what it prints, then return to the detached shape once it starts
  cleanly:

  ```powershell
  dotnet run --project src/PublicApi --no-build 2>&1 | Select-Object -Last 40
  ```
