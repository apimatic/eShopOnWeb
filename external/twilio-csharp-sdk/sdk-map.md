...
All `Source` paths on this map and its sub-pages are relative to the **SDK root** — the directory holding this file and `Twilio.csproj` — never to the page that carries them. Open them as-is from the SDK root, from any page; if the SDK sits under a subdirectory of a larger repo, prefix that subdirectory.
...
var client = new TwilioClient(httpClient, options);
```

DI alternative (`services.AddTwilioClient`):
...
Every API group is a property on the client (e.g. `client.Api20100401Account`). Source: `TwilioClient.cs`. The only constructor is `TwilioClient(HttpClient httpClient, TwilioClientOptions options)`.
...
`RetryOptions` members (namespace `Twilio.Core.Configuration` — add `using Twilio.Core.Configuration;`; source: `Core/Configuration/RetryOptions.cs`; all members are `required`, so build a full instance or start from `RetryOptions.Default()`):
...
| --- | --- | --- |
| `client` (root) | 11 | [map/operations/TwilioClient.md](map/operations/TwilioClient.md) |
...
| Unions (`OneOf`) — variant factories + `TryGet…` | 1 | `Models/OneOf/` |
| Unions (`AnyOf`) — variant factories + `TryGet…` | 3 | `Models/AnyOf/` |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — C# member names + wire values | 405 | `Models/Enums/` |
...
| OneOf unions (`Models/OneOf/`) | `Twilio.Models.OneOf` |
| AnyOf unions (`Models/AnyOf/`) | `Twilio.Models.AnyOf` |
