...
public sealed class StudioV1Execution
{
    private readonly RawClient _rawClient;
    private readonly Server _server;
    private readonly AuthSchemes _auth;

    internal StudioV1Execution(RawClient rawClient, Server server, AuthSchemes auth)
    {
    ...
    /// <returns>A <see cref="Task{TResult}"/> of <see cref="StudioV1FlowExecution"/> instance.</returns>
    /// <exception cref="SdkException{TResult}"> of <see cref="RawError"/> when the server returns an error response.</exception>
    /// <remarks>
    /// Triggers a new Execution for the Flow
    /// </remarks>
    public Task<StudioV1FlowExecution> CreateExecution(string flowSid,
        string to,
        string from,
        object? parameters,
        RequestOptions? requestOptions = null,
        CancellationToken ct = default) =>
        _rawClient.Execute(_server.Default11("/v1/Flows/{FlowSid}/Executions"),
            ...
            JsonResponse.Create<StudioV1FlowExecution>(),
    ...
    /// Delete the Execution and all Steps relating to it.
    /// </remarks>
    public Task DeleteExecution(string flowSid,
        string sid,
        RequestOptions? requestOptions = null,
        CancellationToken ct = default) =>
        _rawClient.Execute(_server.Default11("/v1/Flows/{FlowSid}/Executions/{Sid}"),
    ...
    /// Retrieve an Execution
    /// </remarks>
    public Task<StudioV1FlowExecution> FetchExecution(string flowSid,
        string sid,
        RequestOptions? requestOptions = null,
        CancellationToken ct = default) =>
        _rawClient.Execute(_server.Default11("/v1/Flows/{FlowSid}/Executions/{Sid}"),
    ...
    /// <param name="requestOptions">Per-request options, such as an overriding log level for this call</param>
    /// <param name="ct">Cancellation token</param>
    /// <returns>A <see cref="Task{TResult}"/> of <see cref="ListExecutionResponse"/> instance.</returns>
    /// <exception cref="SdkException{TResult}"> of <see cref="RawError"/> when the server returns an error response.</exception>
    /// <remarks>
    /// Retrieve a list of all Executions for the Flow.
    /// </remarks>
    public Task<ListExecutionResponse> ListExecution(string flowSid,
        ...
        _rawClient.Execute(_server.Default11("/v1/Flows/{FlowSid}/Executions"),
            ...
            HttpMethod.Get,
            EmptyBody.Instance,
            JsonResponse.Create<ListExecutionResponse>(),
            RawErrorResponse.Instance,
            [_auth.AccountSidAuthToken],
            requestOptions,
            ct);
    ...
    /// Update the status of an Execution to <c>ended</c>.
    /// </remarks>
    public Task<StudioV1FlowExecution> UpdateExecution(string flowSid,
        string sid,
        ExecutionEnumStatus status,
        RequestOptions? requestOptions = null,
        CancellationToken ct = default) =>
        _rawClient.Execute(_server.Default11("/v1/Flows/{FlowSid}/Executions/{Sid}"),
            [new TemplateParam("FlowSid", flowSid), new TemplateParam("Sid", sid)],
