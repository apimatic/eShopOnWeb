...
public sealed class ScrapeAndExtractFromUrlsError : ApiError
{
    ...
    private ScrapeAndExtractFromUrlsError(Optional<BatchScrape402Error1> batchScrape402Error1Value,
    ...
    private static ScrapeAndExtractFromUrlsError AsBatchScrape402Error1(BatchScrape402Error1 value) =>
        new(Optional<BatchScrape402Error1>.Some(value), default, default, default);

    private static ScrapeAndExtractFromUrlsError AsBatchScrape429Error1(BatchScrape429Error1 value) =>
        new(default, Optional<BatchScrape429Error1>.Some(value), default, default);

    private static ScrapeAndExtractFromUrlsError AsBatchScrape500Error1(BatchScrape500Error1 value) =>
        new(default, default, Optional<BatchScrape500Error1>.Some(value), default);

    private static ScrapeAndExtractFromUrlsError AsFallback(RawError value) =>
    ...
    internal static Task<ScrapeAndExtractFromUrlsError> Create(HttpResponseMessage response, CancellationToken ct) =>
...
internal sealed class ScrapeAndExtractFromUrlsErrorResponse : IErrorResponse<ScrapeAndExtractFromUrlsError>
{
    public static ScrapeAndExtractFromUrlsErrorResponse Instance { get; } = new();

    private ScrapeAndExtractFromUrlsErrorResponse()
    {
    }

    public Task<ScrapeAndExtractFromUrlsError> Map(HttpResponseMessage response, CancellationToken ct) =>
        ScrapeAndExtractFromUrlsError.Create(response, ct);
}
