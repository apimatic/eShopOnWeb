...
public class SubscriptionActivatedHandler : INotificationHandler<SubscriptionActivated>
{
    ...
    {
        _logger.LogInformation("Subscription {0} activated for user {1} on plan {2}", notification.SubscriptionId, notification.UserId, notification.ProductHandle);

        await _emailSender.SendEmailAsync(
            notification.UserId,
            "Your eShopOnWeb subscription is active",
            $"You are now subscribed to {notification.ProductHandle} (subscription #{notification.SubscriptionId}).");
    }
}
