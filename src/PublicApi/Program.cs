...
builder.Services.AddHttpClient<MaxioApiClient>((sp, client) =>
{
    ...
    if (string.IsNullOrWhiteSpace(maxioOptions.ApiKey) || string.IsNullOrWhiteSpace(maxioOptions.Subdomain))
    {
        throw new InvalidOperationException(
            "Maxio:ApiKey and Maxio:Subdomain are not configured. Set them via user-secrets (see MAXIO_API_KEY / MAXIO_SITE_SUBDOMAIN).");
    }
    ...
    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
        "Basic", Convert.ToBase64String(Encoding.ASCII.GetBytes($"{maxioOptions.ApiKey}:x")));
});
builder.Services.AddScoped<IMaxioApiClient>(sp => sp.GetRequiredService<MaxioApiClient>());
