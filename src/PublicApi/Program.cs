...
builder.Services.AddScoped<IShopperContext, HttpShopperContext>();
builder.Services.Configure<MaxioOptions>(builder.Configuration.GetSection(MaxioOptions.SectionName));
builder.Services.AddHttpClient<ISubscriptionBillingGateway, MaxioBillingGateway>((serviceProvider, client) =>
{
    var maxioOptions = serviceProvider.GetRequiredService<IOptions<MaxioOptions>>().Value;
    MaxioBillingGateway.ConfigureHttpClient(client, maxioOptions);
});
