...
// Maxio Advanced Billing subscription billing (billing system of record for subscriptions).
...
builder.Services.AddHttpClient(MaxioClient.HttpClientName, (sp, httpClient) =>
{
    var maxioOptions = sp.GetRequiredService<IOptions<MaxioOptions>>().Value;
    httpClient.BaseAddress = maxioOptions.ResolveBaseAddress();
    httpClient.Timeout = TimeSpan.FromSeconds(30);
});
builder.Services.AddTransient<IMaxioClient, MaxioClient>();
builder.Services.AddScoped<IMaxioSubscriptionService>(sp =>
    new MaxioSubscriptionService(
        sp.GetRequiredService<IMaxioClient>(),
        sp.GetRequiredService<IOptions<MaxioOptions>>().Value));
