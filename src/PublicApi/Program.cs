...
// The deployment-facing environment variable names are deliberately mapped to the
// configuration section consumed by the integration. Values remain in memory only.
...
builder.Services.AddOptions<PayPalOptions>()
    ...
    .Validate(x => !string.IsNullOrWhiteSpace(x.BaseUrl) ||
        string.Equals(x.Environment, "sandbox", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(x.Environment, "live", StringComparison.OrdinalIgnoreCase),
