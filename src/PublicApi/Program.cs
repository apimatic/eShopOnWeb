...
builder.Services.AddHttpClient<IPhoneNumberLookup, TwilioLookupClient>((sp, client) =>
{
    ...
    TwilioLookupClient.ConfigureClient(client, settings);
});
builder.Services.AddHttpClient<ISmsMessageGateway, TwilioMessagingClient>((sp, client) =>
{
    var settings = sp.GetRequiredService<IOptions<TwilioSettings>>().Value;
    TwilioMessagingClient.ConfigureClient(client, settings);
});
