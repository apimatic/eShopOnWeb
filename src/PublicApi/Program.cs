...
builder.Services.AddHttpClient<ITwilioMessageClient, TwilioMessageClient>(client =>
{
    client.Timeout = TimeSpan.FromSeconds(30);
});
...
static void BindTwilioEnvironmentVariables(ConfigurationManager configuration)
{
    ...
    MapIfPresent(overlay, "TWILIO_BASE_URL", "Twilio:BaseUrl");
...
static void MapIfPresent(IDictionary<string, string?> overlay, string environmentVariable, string configurationKey)
{
