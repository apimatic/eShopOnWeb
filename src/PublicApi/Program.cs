...
// SMS order notifications (Twilio). Settings are bound from the Twilio: configuration section; no
// value is hard-coded. The typed HttpClient has its built-in loggers removed so that a shopper's
// number (which appears in a lookup URL) can never reach the logs.
