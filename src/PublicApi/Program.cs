...
builder.Services.AddHttpClient<IPhoneNumberLookupService, TwilioLookupClient>();
builder.Services.AddHttpClient<ITwilioMessagingService, TwilioMessagingClient>();
...
static void BindTwilioEnvironment(ConfigurationManager configuration)
{
    ...
    void Map(string environmentVariable, string configurationKey)
    {
        ...
        if (!string.IsNullOrWhiteSpace(value) && string.IsNullOrWhiteSpace(configuration[configurationKey]))
        {
