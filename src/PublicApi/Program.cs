...
builder.Services.AddScoped<IOrderNotificationOrchestrator, OrderNotificationOrchestrator>();

builder.Services.Configure<TwilioSettings>(builder.Configuration.GetSection(TwilioSettings.SectionName));
builder.Services.AddHttpClient<IPhoneNumberLookupService, TwilioLookupService>(client =>
{
    client.Timeout = TimeSpan.FromSeconds(30);
});
builder.Services.AddHttpClient<ISmsGateway, TwilioSmsGateway>(client =>
{
    client.Timeout = TimeSpan.FromSeconds(30);
});
