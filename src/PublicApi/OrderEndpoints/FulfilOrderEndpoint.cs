...
/// <summary>
/// Operator action: marks the order fulfilled, which is when the held money is actually captured.
/// A stale authorization is renewed first; one that can no longer be renewed is reported to the operator.
/// </summary>
public class FulfilOrderEndpoint : IEndpoint<IResult, FulfilOrderRequest, IOrderPaymentService>
{
    ...
    {
        await service.FulfilAsync(request.OrderId);
