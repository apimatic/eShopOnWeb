...
/// The response shows what PayPal reported: captured amount, PayPal's fee, net proceeds.
/// A stale authorization is renewed automatically; one that cannot be renewed
/// fails with an operator-actionable error.
/// </summary>
public class FulfilOrderEndpoint : IEndpoint<IResult, FulfilOrderRequest, IOrderPaymentService>
{
    ...
    {
        var payment = await orderPaymentService.FulfilAsync(request.OrderId);
