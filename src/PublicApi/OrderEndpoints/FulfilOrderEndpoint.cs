...
/// <summary>
/// Operator: marks the order fulfilled and captures the held funds. The response reports what
/// PayPal returned: captured amount, PayPal's fee and the net proceeds. A stale authorization
/// is renewed first; one that can no longer be renewed fails with an actionable error.
/// </summary>
public class FulfilOrderEndpoint : IEndpoint<IResult, FulfilOrderRequest, IPaymentService>
{
    ...
    public async Task<IResult> HandleAsync(FulfilOrderRequest request, IPaymentService paymentService)
    {
        ...
        var payment = await paymentService.FulfilAsync(request.OrderId, default);
        if (payment == null)
        {
