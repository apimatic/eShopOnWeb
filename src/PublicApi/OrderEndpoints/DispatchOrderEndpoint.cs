...
public class DispatchOrderEndpoint : IEndpoint<IResult, DispatchOrderRequest, IOrderNotificationService>
{
    public void AddRoute(IEndpointRouteBuilder app)
    {
        app.MapPost("api/orders/{orderId}/dispatch",
            ...
            (int orderId, IOrderNotificationService orderNotificationService) =>
            {
                return await HandleAsync(new DispatchOrderRequest(orderId), orderNotificationService);
            })
            .Produces<DispatchOrderResponse>()
            .WithTags("OrderEndpoints");
    }

    public async Task<IResult> HandleAsync(DispatchOrderRequest request, IOrderNotificationService orderNotificationService)
    {
        var order = await orderNotificationService.DispatchAsync(request.OrderId);
        if (order == null)
        {
