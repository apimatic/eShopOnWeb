...
public class DispatchOrderEndpoint : IEndpoint
{
    ...
    {
        var result = await service.DispatchOrderAsync(orderId, ct);
        return result switch
        {
            OrderTransition.NotFound => Results.NotFound(),
            OrderTransition.NoChange => Results.Ok(new { orderId, status = "Dispatched", changed = false }),
            _ => Results.Ok(new { orderId, status = "Dispatched", changed = true })
        };
    }
}
