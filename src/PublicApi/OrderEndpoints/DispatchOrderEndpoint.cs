...
/// delivery-feedback follow-up is queued with the provider for a few days later.
/// </summary>
public class DispatchOrderEndpoint : IEndpoint<IResult, int>
{
    ...
    {
        var dispatched = await _orderNotificationService.DispatchAsync(orderId);
        return dispatched
            ? Results.Ok(new DispatchOrderResponse { OrderId = orderId, Status = "dispatched" })
            : Results.NotFound();
    }
}
