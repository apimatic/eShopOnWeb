    ...
    public void AddRoute(IEndpointRouteBuilder app)
    {
        app.MapPost("api/orders/{orderId:int}/dispatch",
            ...
            (int orderId, IOrderNotificationService service, CancellationToken ct) =>
            {
                ...
                {
                    OrderActionResult.Success => Results.Ok(new OrderActionResponse { OrderId = orderId, Status = "Dispatched" }),
                    OrderActionResult.NotFound => Results.NotFound(),
                    OrderActionResult.InvalidState => Results.Conflict(new { message = "The order cannot be dispatched from its current state." }),
