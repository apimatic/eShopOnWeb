...
public class ListOrderNotificationsEndpoint : IEndpoint<IResult, int, ICatalogOrderService>
{
    ...
    private async Task<IResult> HandleAsync(int orderId, ClaimsPrincipal user, ICatalogOrderService orders)
    {
        ...
        var order = await orders.GetByIdAsync(orderId, default);
        if (order is null)
        {
        ...
        if (!isAdmin && !order.BelongsTo(buyerId))
        {
            return Results.NotFound();
        }

        var notifications = await _notifications.ListForOrderAsync(orderId, default);
