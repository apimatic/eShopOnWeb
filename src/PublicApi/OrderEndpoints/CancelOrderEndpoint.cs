...
/// <summary>
/// Operator action: cancels the order before fulfilment. The shopper's held funds
/// are released (the authorization is voided); no money ever moves.
/// </summary>
public class CancelOrderEndpoint : IEndpoint<IResult, CancelOrderRequest, IOrderPaymentService>
{
    ...
    {
        var payment = await orderPaymentService.CancelAsync(request.OrderId);
