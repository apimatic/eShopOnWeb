...
/// <summary>
/// Operator action: cancels an order. The shopper is told, and any follow-up
/// message still scheduled with the provider is cancelled so it never goes out.
/// </summary>
public class CancelOrderEndpoint : IEndpoint<IResult, CancelOrderRequest>
{
    ...
    public async Task<IResult> HandleAsync(CancelOrderRequest request)
    {
        ...
        {
            return Results.Conflict(new { error = $"Order {order.Id} is already cancelled." });
        }

        order.MarkCancelled();
        await _orderRepository.UpdateAsync(order);

        // Best-effort: messaging problems must never fail the cancellation.
