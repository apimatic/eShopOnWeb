...
/// <summary>
/// Operator action: cancel an order. The shopper is told, and any delivery follow-up that has not yet gone
/// out is called off so it never reaches them.
/// </summary>
public class CancelOrderEndpoint : IEndpoint<IResult, int, IOrderNotificationService>
{
    ...
    public async Task<IResult> HandleAsync(int orderId, IOrderNotificationService service)
    {
        ...
        return cancelled
            ? Results.Ok(new OrderActionResponse { OrderId = orderId, Action = "cancelled" })
            : Results.NotFound();
    }
}
