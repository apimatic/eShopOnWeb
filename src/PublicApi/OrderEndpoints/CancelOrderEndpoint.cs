...
/// <summary>
/// Operator: cancels an order before fulfilment, releasing the shopper's held funds.
/// </summary>
public class CancelOrderEndpoint : IEndpoint<IResult, CancelOrderRequest, IPaymentService>
{
    public void AddRoute(IEndpointRouteBuilder app)
    {
        app.MapPost("api/orders/{orderId}/cancel",
            ...
            {
                return await HandleAsync(new CancelOrderRequest(orderId), paymentService, cancellationToken);
            })
    ...
    private async Task<IResult> HandleAsync(CancelOrderRequest request, IPaymentService paymentService, CancellationToken cancellationToken)
    {
        var payment = await paymentService.CancelOrderAsync(request.OrderId, cancellationToken);
