...
/// starting a sync can return immediately while the import proceeds in the background.
/// </summary>
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                await service.RunSyncAsync(syncId, stoppingToken);
            }
            catch (Exception ex)
            {
                // RunSyncAsync records its own failures; this guards against anything unexpected so
