...
/// <summary>
/// Registers a supplier: a name and the URL of its product listing page. Administrator-only.
/// </summary>
public class RegisterSupplierEndpoint : IEndpoint<IResult, RegisterSupplierRequest, IRepository<Supplier>>
{
    ...
    public async Task<IResult> HandleAsync(RegisterSupplierRequest request, IRepository<Supplier> supplierRepository)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.ListingUrl) ||
            !Uri.TryCreate(request.ListingUrl, UriKind.Absolute, out var uri) ||
            (uri.Scheme != Uri.UriSchemeHttp && uri.Scheme != Uri.UriSchemeHttps))
        {
            return Results.BadRequest("A valid http(s) listing URL is required.");
        }

        var supplier = new Supplier(request.Name, request.ListingUrl);
