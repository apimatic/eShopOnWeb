...
/// <summary>
/// Puts a bill to the shopper (operator action). Afterwards a way to pay it can be handed out and the
/// bill reports itself as having been put to them.
/// </summary>
public class IssueInvoiceEndpoint : IEndpoint<IResult, InvoiceRef, IInvoiceService>
{
    ...
    {
        var result = await invoiceService.IssueAsync(request.BuyerId, request.IsOperator, request.InvoiceId);
