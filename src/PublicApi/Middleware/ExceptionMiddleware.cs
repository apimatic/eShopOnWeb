...
public class ExceptionMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        {
            // TwilioProviderException messages are caller-safe by construction (no provider
            // bodies, no phone numbers); the status ladder keeps our own auth/quota faults
            // (401/403/429 at the provider) from being misreported as the caller's fault.
            context.Response.StatusCode = (int?)providerException.StatusCode switch
            {
                401 or 403 => (int)HttpStatusCode.BadGateway,
                429 => (int)HttpStatusCode.ServiceUnavailable,
                >= 400 and < 500 => (int)providerException.StatusCode!,
