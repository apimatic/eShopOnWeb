...
public class ExceptionMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        else if (exception is PayerActionRequiredException payerActionRequiredException)
        {
            context.Response.StatusCode = (int)HttpStatusCode.BadGateway;
            await context.Response.WriteAsync(new ErrorDetails()
            {
                StatusCode = context.Response.StatusCode,
                Message = payerActionRequiredException.Message
            }.ToString());
        }
        else if (exception is PaymentGatewayException paymentGatewayException)
        {
            // Carry a caller-actionable provider 4xx; anything else is a provider-side failure.
