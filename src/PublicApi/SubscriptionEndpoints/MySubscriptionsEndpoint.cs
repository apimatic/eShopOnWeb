...
public class MySubscriptionsEndpoint : IEndpoint<IResult, MySubscriptionsRequest, SubscriptionEndpointContext>
{
    public void AddRoute(IEndpointRouteBuilder app)
    {
        app.MapGet("api/subscriptions/mine",
            ...
            {
                return await HandleAsync(new MySubscriptionsRequest(), SubscriptionEndpointContext.From(subscriptionService, user));
            })
    ...
    public async Task<IResult> HandleAsync(MySubscriptionsRequest request, SubscriptionEndpointContext context)
    {
        ...
        var subscriptions = await context.SubscriptionService.GetSubscriptionsForUserAsync(context.UserId);
        foreach (var subscription in subscriptions)
        {
            response.Subscriptions.Add(SubscriptionMapping.ToDto(subscription));
        }
