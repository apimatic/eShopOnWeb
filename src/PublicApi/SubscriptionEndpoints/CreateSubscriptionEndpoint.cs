...
public class CreateSubscriptionEndpoint : IEndpoint<IResult>
{
    ...
    public CreateSubscriptionEndpoint(SubscriptionsService subscriptionsService)
    {
    ...
    {
        // This method signature is required by IEndpoint<IResult> but we handle the logic in the MapPost lambda
    ...
    private async Task<IResult> HandleAsync(CreateSubscriptionRequest request, HttpContext httpContext)
    {
        ...
        try
        {
            ...
            // Create subscription
            var subscription = await _subscriptionsService.CreateSubscriptionAsync(
                customerId, userId, request.PlanHandle);
