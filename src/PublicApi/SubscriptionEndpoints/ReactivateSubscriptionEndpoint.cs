...
/// <summary>Reactivates a cancelled/unpaid/expired subscription (UC4). A caller acts on their own subscription; an Administrator may act on any.</summary>
public class ReactivateSubscriptionEndpoint : IEndpoint<IResult, LifecycleActionRequest, ISubscriptionService>
{
    ...
    public async Task<IResult> HandleAsync(LifecycleActionRequest request, ISubscriptionService subscriptionService)
    {
        ...
        var updated = await subscriptionService.ReactivateAsync(request.SubscriptionId, request.OwnerReference);
