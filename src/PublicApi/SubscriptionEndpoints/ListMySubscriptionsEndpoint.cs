...
public class ListMySubscriptionsEndpoint : IEndpoint<IResult, ISubscriptionBillingService>
{
    ...
    {
        var shopper = await BillingShopperResolver.ResolveAsync(
        ...
        var subscriptions = await billing.GetMySubscriptionsAsync(shopper);
        response.Subscriptions.AddRange(subscriptions.ToSubscriptionDtos());
