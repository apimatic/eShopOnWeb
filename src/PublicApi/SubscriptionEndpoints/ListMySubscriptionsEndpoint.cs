...
public class ListMySubscriptionsEndpoint : IEndpoint<IResult, ISubscriptionBillingService>
{
    ...
    public async Task<IResult> HandleAsync(ISubscriptionBillingService billing)
    {
        ...
        var subscriptions = await billing.ListMySubscriptionsAsync(shopper, httpContext.RequestAborted);
        response.Subscriptions.AddRange(SubscriptionMappings.ToDtos(subscriptions));
