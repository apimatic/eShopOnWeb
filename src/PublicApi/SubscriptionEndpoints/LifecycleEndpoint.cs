...
/// <summary>
/// Pause, resume, cancel or reactivate a subscription
/// </summary>
public class LifecycleEndpoint : IEndpoint<IResult, LifecycleRequest, ISubscriptionService>
{
    ...
    public async Task<IResult> HandleAsync(LifecycleRequest request, ISubscriptionService subscriptionService)
    {
        ...
        var action = SubscriptionRequestParser.ParseAction(request.Action);

        var subscription = await subscriptionService.ApplyLifecycleActionAsync(request.UserReference,
            request.SubscriptionId, action, request.Reason);
