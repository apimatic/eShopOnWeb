...
/// <summary>Resumes a paused subscription (UC4). A caller acts on their own subscription; an Administrator may act on any.</summary>
public class ResumeSubscriptionEndpoint : IEndpoint<IResult, LifecycleActionRequest, ISubscriptionService>
{
    ...
    public async Task<IResult> HandleAsync(LifecycleActionRequest request, ISubscriptionService subscriptionService)
    {
        ...
        var updated = await subscriptionService.ResumeAsync(request.SubscriptionId, request.OwnerReference);
