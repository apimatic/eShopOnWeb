...
/// <summary>
/// Enrolls the calling user in a plan (UC1). The customer reference is always the caller's own
/// identity (§4.4) — there is no "subscribe on behalf of another user".
/// </summary>
public class SubscribeEndpoint : IEndpoint<IResult, SubscribeRequest, ISubscriptionService>
{
    ...
    public async Task<IResult> HandleAsync(SubscribeRequest request, ISubscriptionService subscriptionService)
    {
        ...
        var subscription = await subscriptionService.SubscribeAsync(request.BuyerId, request.BuyerId, request.ProductHandle);
