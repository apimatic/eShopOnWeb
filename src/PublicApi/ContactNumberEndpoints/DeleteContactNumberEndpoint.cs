...
/// <summary>
/// Removes one of the signed-in shopper's registered numbers. A shopper can only
/// remove their own numbers.
/// </summary>
public class DeleteContactNumberEndpoint : IEndpoint<IResult, int, ClaimsPrincipal>
{
    ...
    public async Task<IResult> HandleAsync(int contactNumberId, ClaimsPrincipal user)
    {
        ...
        var contactNumber = await _contactNumberRepository.FirstOrDefaultAsync(new ContactNumberByIdSpecification(contactNumberId));
