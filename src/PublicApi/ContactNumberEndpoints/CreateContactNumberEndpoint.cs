...
public class CreateContactNumberEndpoint : IEndpoint<IResult, CreateContactNumberRequest, IContactNumberService>
{
    ...
    public async Task<IResult> HandleAsync(CreateContactNumberRequest request, HttpContext httpContext, IContactNumberService service)
    {
        try
        {
            var contact = await service.RegisterAsync(httpContext.GetBuyerId(), request.PhoneNumber, request.CountryCode);
