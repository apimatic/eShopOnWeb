...
public class CreateContactNumberEndpoint : IEndpoint<IResult, CreateContactNumberRequest, IContactNumberService>
{
    ...
    private async Task<IResult> HandleAsync(CreateContactNumberRequest request, ClaimsPrincipal user, IContactNumberService contactNumbers)
    {
        var response = new CreateContactNumberResponse(request.CorrelationId());
        var created = await contactNumbers.RegisterAsync(BuyerIdentity.RequireBuyerId(user), request.PhoneNumber);
        response.ContactNumberId = created.Id;
        response.CanonicalNumber = created.CanonicalNumber;
