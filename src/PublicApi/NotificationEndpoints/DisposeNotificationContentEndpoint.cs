...
public class DisposeNotificationContentEndpoint : IEndpoint<IResult, DisposeNotificationContentRequest, IOrderNotificationService>
{
    ...
    public async Task<IResult> HandleAsync(DisposeNotificationContentRequest request, IOrderNotificationService notifications)
    {
        var updated = await notifications.DisposeContentAsync(request.NotificationId);
        return Results.Ok(NotificationDto.From(updated));
    }
}
