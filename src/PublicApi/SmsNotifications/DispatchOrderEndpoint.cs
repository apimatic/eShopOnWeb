...
/// <summary>
/// POST /api/orders/{orderId}/dispatch — operator marks the order dispatched. The shopper is told it is on
/// its way, and a delivery-feedback follow-up is queued with the provider for a few days later.
/// </summary>
    ...
    : IEndpoint<IResult, int, IOrderNotificationService, HttpContext>
{
    ...
    {
        var result = await service.DispatchAsync(orderId, http.RequestAborted);
        return result.Status switch
        {
            OrderActionStatus.NotFound => Results.NotFound(),
            OrderActionStatus.InvalidState => Results.Conflict(new { error = result.Message }),
            _ => Results.Ok(new OrderActionResponse { OrderId = orderId, Status = "Dispatched" })
        };
    }
}
