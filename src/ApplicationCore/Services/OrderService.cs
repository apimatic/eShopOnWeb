...
public class OrderService : IOrderService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (items.Count == 0)
        {
            throw new CatalogItemUnavailableException("An order must contain at least one catalog item.");
        }
        ...
        foreach (var line in items)
        {
            if (line.Quantity <= 0)
            {
                throw new CatalogItemUnavailableException("Each order line must have a quantity greater than zero.");
            }
        }
        ...
        if (missing.Count > 0)
        {
            throw new CatalogItemUnavailableException($"Catalog item(s) not found: {string.Join(", ", missing)}.");
        }
        ...
        var orderItems = items.Select(line =>
        {
            ...
            {
                pictureUri = catalogItem.PictureUri ?? "placeholder";
            }
