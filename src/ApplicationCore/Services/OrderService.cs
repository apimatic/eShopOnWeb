...
public class OrderService : IOrderService
{
    ...
    public async Task<Order> CreateOrderAsync(string buyerId, IEnumerable<CatalogItemQuantity> items, Address shippingAddress)
    {
        Guard.Against.NullOrEmpty(buyerId, nameof(buyerId));
        var requestedItems = items?.ToList() ?? new List<CatalogItemQuantity>();
        ...
        var orderItems = requestedItems.Select(requested =>
        {
            ...
            Guard.Against.Null(catalogItem, nameof(catalogItem), $"No catalog item found for id {requested.CatalogItemId}.");
