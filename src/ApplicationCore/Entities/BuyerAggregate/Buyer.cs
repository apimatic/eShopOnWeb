...
public class Buyer : BaseEntity, IAggregateRoot
{
    ...
    public void AssignPayPalCustomerId(string payPalCustomerId)
    {
        Guard.Against.NullOrEmpty(payPalCustomerId, nameof(payPalCustomerId));
        PayPalCustomerId = payPalCustomerId;
    }

    public PaymentMethod AddPaymentMethod(string vaultId, string last4, string? brand, string? expiry, string? alias)
    {
        var method = new PaymentMethod(vaultId, last4, brand, expiry, alias);
    ...
    {
        var existing = GetPaymentMethod(paymentMethodId);
        if (existing is null)
        {
            return false;
        }
