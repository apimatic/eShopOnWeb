...
Register the client with `IServiceCollection` and resolve it from the container. The `HttpClient` is managed by `IHttpClientFactory`. Configure the client's behavior through [TwilioSdkClientOptions](TwilioSdkClientOptions.cs).
...
Create the client by passing an `HttpClient` you manage yourself. Configure the client's behavior through [TwilioSdkClientOptions](TwilioSdkClientOptions.cs).
