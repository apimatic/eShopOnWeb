...
file static class JsonCompatibility
{
...
public sealed class AdditionalProperties : IDictionary<string, JsonElement>, IEquatable<AdditionalProperties>
{
    ...
    public void Set(string key, object? value) =>
        _data[key] = JsonSerializer.SerializeToElement(value, JsonCompatibility.WebOptions);

    public bool TryGetValue<T>(string key, [NotNullWhen(true)] out T? value)
    {
        value = default;
        return _data.TryGetValue(key, out var element) &&
               JsonSerializer.TryDeserialize(element, JsonCompatibility.WebOptions, out value);
    }
...
public sealed class AdditionalProperties<TValue> : IDictionary<string, JsonElement>, IEquatable<AdditionalProperties<TValue>>
{
    ...
    public TValue this[string key]
    {
        ...
        set => _data[key] = JsonSerializer.SerializeToElement(value, JsonCompatibility.WebOptions);
    }
...
file static class JsonElementExtensions
{
    extension(Dictionary<string, JsonElement> data)
    {
        public bool DeepEquals(Dictionary<string, JsonElement> other)
        {
            ...
            {
                if (!other.TryGetValue(pair.Key, out var element) || !JsonCompatibility.DeepEquals(pair.Value, element))
                    return false;
            }
        ...
        public bool DeepContains(KeyValuePair<string, JsonElement> pair) =>
            ...
            data.TryGetValue(pair.Key, out var existing) && JsonCompatibility.DeepEquals(existing, pair.Value);
    }
    ...
    extension(JsonElement element)
    {
        ...
        public T ReadRequired<T>(string key) =>
            element.Deserialize<T>(JsonCompatibility.WebOptions)
