...
public sealed class GetBillingPlanError : ApiError
{
    ...
    private GetBillingPlanError(Optional<SubscriptionError> subscriptionErrorValue,
    ...
    private static GetBillingPlanError AsFallback(RawError value) => new(default, Optional<RawError>.Some(value));

    public bool TryGetSubscriptionError(out SubscriptionError value) => _subscriptionErrorValue.TryGetValue(out value);

    private static Task<GetBillingPlanError> Create(FailedResponse response) =>
    ...
    internal static ApiErrorResponse<GetBillingPlanError> Response { get; } = new(Create);
}
