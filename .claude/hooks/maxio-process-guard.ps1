...
# [macos fork] `kill` and `pgrep` join the intent set. On macOS the dangerous shape is
# `kill $(pgrep -f PublicApi)` / `pgrep -f dotnet | xargs kill`, which the Windows list does not
# match at all -- pkill/killall were already covered, bare kill+pgrep were not.
$killIntent = $cmd -match '(?i)(\bStop-Process\b|\btaskkill\b|\bpkill\b|\bkillall\b|\bkill\b|\bpgrep\b)'
...
$nameSelected =
    ...
    ($cmd -match '(?i)\b(pkill|killall)\b') -or
    # [macos fork] pgrep/ps-grep select by name or command-line pattern exactly as -IM does.
    ($cmd -match '(?i)\bpgrep\b') -or
    ($cmd -match '(?i)\bps\b[^;|\r\n]*\|[^;|\r\n]*\bgrep\b')
...
$enumerates = $cmd -match '(?i)(\bGet-Process\b|Win32_Process|\bGet-WmiObject\b|\bGet-CimInstance\b|\bpgrep\b|\bps\s+(-\w+\s*)*(aux|ax|-ef)\b)'

# The sanctioned anchor: a PID resolved from a port. The run owns APP_PORT_BLOCK_BASE..+SIZE-1.
# [macos fork] Get-NetTCPConnection DOES NOT EXIST on macOS, so the Windows guard would deny the
# kill and then recommend an impossible command. `lsof -ti:<port>` is the posix equivalent anchor:
# it names a port this run owns, not an image name.
$portAnchored =
    (($cmd -match '(?i)Get-NetTCPConnection') -and ($cmd -match '(?i)(LocalPort|OwningProcess)')) -or
    ($cmd -match '(?i)\blsof\b[^;|\r\n]*-[a-z]*t[a-z]*i[^;|\r\n]*:\s*\d{2,5}') -or
    ($cmd -match '(?i)\blsof\b[^;|\r\n]*-i\b[^;|\r\n]*:\s*\d{2,5}')
...
`APP_PORT_BLOCK_BASE + APP_PORT_BLOCK_SIZE - 1`):
  ...
  # macOS -- stop only what is listening on YOUR port
  lsof -ti:<your-port> | xargs kill

  # (Get-NetTCPConnection is Windows-only and is NOT available on this machine.)
