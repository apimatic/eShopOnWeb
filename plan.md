...
A dedicated Product Family has been seeded on the Maxio site **apimatic-hackathon** for this integration. All UCs operate against the entities below. Use these exact handles and IDs in configuration and code — they are the verified, live identifiers in the sandbox. **Note — handles are stable, numeric IDs are not:** Maxio assigns the numeric IDs, and they are reassigned whenever the catalog is re-created (e.g. a sandbox re-seed), so the IDs shown here are illustrative of the current live state, not permanent. The **handles** (`eshop-subscribe`, `eshop-pro`, `basic-plan`, `api-call`) are the durable identifiers — resolve each entity's live ID from its handle at startup rather than relying on a hard-coded numeric ID.
...
- Product family `eshop-subscribe` (id 3023074) exists and is reachable with the configured API key.
- Products `eshop-pro` (id 7126957, $299/mo) and `basic-plan` (id 7126958, $29/mo) exist inside that family, both with requires payment method = off.
- Metered component `api-call` (id 3057195) exists on that family with kind Metered and default price $0.01/unit.
...
**Phase 0 — Seed the billing-provider sandbox (UC0).** Ensure the product family, plans, and metered component exist on the sandbox with the configured handles. Current sandbox state: family `eshop-subscribe` (id 3023074), products `eshop-pro` (id 7126957) and `basic-plan` (id 7126958), metered component `api-call` (id 3057195) — all present, no action needed unless reseeding from scratch. *Verify:* Reading back the family shows the two products and the component; the integration's API key can list them.
...
- **Second plan for UC3:** `basic-plan` (id 7126958, $29/mo) seeded alongside `eshop-pro` in family `eshop-subscribe`.
- **Metered component for UC2:** `api-call` (id 3057195) seeded on family `eshop-subscribe`, type Metered, $0.01/unit. Component is on the family so it's automatically available to every subscription on either plan — no per-subscribe attach step needed.
