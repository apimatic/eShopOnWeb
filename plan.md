...
> **Benchmark-run variant.** Same eShopOnWeb + Maxio integration plan as the standard build, with **live Maxio sandbox testing omitted**. The build is validated against a local, provider-faithful readiness harness — not the live Maxio site — so there is **no live-sandbox seeding, no read-back verification against the real site, and no live-key exercise of the use cases**. Build the integration exactly as specified (same architecture, provider seam, use cases, and the configurable outbound target server — §2.3/§4.3/§5, still a hard requirement: point the client at the endpoint the run provides). The pricing model, handles, and IDs below are the **reference contract the integration targets**; the catalog is already provisioned, so you neither seed nor verify it. Live credentials, if present, are not used. Everything else in this plan is unchanged.
...
### 1.3 Reference pricing model (the catalog the integration targets)

A dedicated Product Family defines the catalog this integration targets. All UCs operate against the entities below. Use these exact handles and IDs in configuration and code. **Note — handles are stable, numeric IDs are not:** Maxio assigns the numeric IDs, and they are reassigned whenever the catalog is re-created, so the IDs shown here are illustrative, not permanent. The **handles** (`eshop-subscribe`, `eshop-pro`, `basic-plan`, `api-call`) are the durable identifiers — resolve each entity's ID from its handle at startup rather than relying on a hard-coded numeric ID. (The catalog is already provisioned against the configured target; this build does not seed or verify it.)
...
|------|------|--------|----|-------|---------|
| Pro Plan | Product (recurring monthly) | eshop-pro | 7126957 | $299.00 / month | UC1 (hero subscribe target), UC3 (upgrade target from Basic) |
| Basic Plan | Product (recurring monthly) | basic-plan | 7126958 | $29.00 / month | UC3 (downgrade target from Pro); optional UC1 alternative |
...
UC0 (Catalog) is a pre-provisioned given — every other UC assumes the catalog already exists against the configured target. UC1 (Subscribe) is the mandatory hero flow. UC2 (pay-as-you-go usage) is a priority focus. UC3–UC4 add depth.

### UC0 — Catalog the Integration Targets (pre-provisioned; reference only)

> **Benchmark note:** In this variant the catalog is **already provisioned** against the configured target. This build does **not** create it, **not** read it back, and **not** verify it against a live site. UC0 is reference only — the entities below define what the integration consumes; treat them as a given and move straight to UC1.

The integration depends on a specific set of entities existing in the billing provider before any customer-facing flow can run. Those entities are already provisioned. The definitions below describe exactly what exists so the integration can be configured to match it.

- **Priority:** Given — the reference catalog for UC1–UC4 (not performed by this build)
- **Goal (reference):** Describe the product family, recurring plans, and metered component the integration expects, with the exact handles the integration is configured to use.

**The provisioned entities:**

1. A product family with name `eShopSubscribe` and handle `eshop-subscribe`.
2. The primary recurring plan inside that family — name Pro Plan, handle `eshop-pro`, price $299.00 / month, no trial, no setup fee, expires never, taxable no, payment method NOT required.
3. The alternate recurring plan inside the same family — name Basic Plan, handle `basic-plan`, price $29.00 / month, same other settings (no trial, no setup fee, expires never, taxable no, payment method not required).
4. The metered component on the family — name API Calls, handle `api-call`, kind Metered, pricing scheme per-unit, default price $0.01 / unit.

**Reference state (what the integration is configured against):**

- Product family `eshop-subscribe` (id 3023074).
- Products `eshop-pro` (id 7126957, $299/mo) and `basic-plan` (id 7126958, $29/mo), both with requires payment method = off.
- Metered component `api-call` (id 3057195), kind Metered, default price $0.01/unit, on that family (so it is automatically available to every subscription on either plan).

**Notes for the implementing agent:**

- UC0 is not exposed through the subscription module and is not built or run by this task — it is background context describing the catalog the integration consumes.
- Resolve each entity's live ID from its **handle** (handles are stable; numeric IDs are illustrative). Do not hard-code the numeric IDs.
- Do not attempt to seed, read back, or otherwise verify the catalog against a live Maxio site — the readiness harness supplies the provider behavior the build is validated against.
...
- **Preconditions:**
  ...
  - At least one plan is available from the billing provider — for the demo, Pro Plan, handle `eshop-pro`, id 7126957, in product family `eshop-subscribe` (id 3023074). Basic Plan (`basic-plan`, id 7126958) is also available as an alternative.
...
- Configured product handle does not resolve (stale IDs) → fail with a configuration error pointing back at the reference catalog (UC0); do not enroll against a guessed plan.
...
- Provider rejects enrollment (e.g. plan unexpectedly requires a payment method) → surface the provider's message.
...
- **Preconditions:**
  ...
  - The metered component `api-call` (id 3057195, kind Metered, $0.01/unit) exists on the family and is therefore automatically available to the subscription
...
- The configured component handle resolves to a non-metered component (or does not resolve at all) → the startup/first-call validation refuses to record usage; fix the configuration to match the reference catalog (UC0) before retrying.
...
- **Preconditions:** Customer has an active subscription on either `eshop-pro` (id 7126957) or `basic-plan` (id 7126958); the other plan is available as a target.
...
- Target plan handle does not resolve or the plan is archived → configuration error; point back at the reference catalog (UC0).
...
// SAME build can target prod / dev / a local endpoint — explicit Maxio:BaseUrl wins, else derive
...
- `MaxioSettings.ResolveBaseUrl()` (or equivalent composition-root logic) returns the explicit `Maxio:BaseUrl` when present, otherwise the subdomain-derived host. This is the one place retargeting happens; keeping it here means pointing at a different endpoint is a config change, not a code change.
...
Stored in .NET user-secrets for the host that owns the subscription endpoints (Web and/or PublicApi — both already declare a `UserSecretsId`). Never committed to git. All identifiers below are the values for the reference catalog the integration targets.
...
# --- Target server (CONFIGURABLE: prod / dev / local endpoint) ---
...
# tenant, or a local endpoint — no code change. Leave empty to use the derived host.
Maxio:BaseUrl                   =               # e.g. https://apimatic-hackathon.chargify.com
                                                #      or  http://localhost:8080 (local endpoint, for testing)

# --- Reference catalog entities (Product Family / Products / Metered Component) ---
Maxio:ProductFamilyHandle       = eshop-subscribe
Maxio:ProductFamilyId           = 3023074

Maxio:DefaultProductHandle      = eshop-pro
Maxio:DefaultProductId          = 7126957

Maxio:AlternateProductHandle    = basic-plan
Maxio:AlternateProductId        = 7126958

Maxio:MeteredComponentHandle    = api-call
Maxio:MeteredComponentId        = 3057195
```
...
- **Local endpoint (testing):** set `Maxio:BaseUrl = http://localhost:8080` (or wherever the run's endpoint listens). All outbound Maxio traffic then targets that endpoint; no live calls escape. **Note:** supporting this target is a configuration capability the client must have — it is *not* a task. This plan does not ask the implementer to stand up, invoke, or investigate any endpoint (§7, §8).
...
dotnet user-secrets set "Maxio:BaseUrl"                ""            # empty = derived host; set to override (prod/dev/local)
dotnet user-secrets set "Maxio:ProductFamilyHandle"    "eshop-subscribe"
dotnet user-secrets set "Maxio:ProductFamilyId"        "3023074"
dotnet user-secrets set "Maxio:DefaultProductHandle"   "eshop-pro"
dotnet user-secrets set "Maxio:DefaultProductId"       "7126957"
dotnet user-secrets set "Maxio:AlternateProductHandle" "basic-plan"
dotnet user-secrets set "Maxio:AlternateProductId"     "7126958"
dotnet user-secrets set "Maxio:MeteredComponentHandle" "api-call"
dotnet user-secrets set "Maxio:MeteredComponentId"     "3057195"

# Example: point at a local endpoint instead of the derived host (no secret; env var also works)
...
**Phase 0 — Catalog (pre-provisioned; reference only).** The product family, plans, and metered component already exist against the configured target with the handles below — family `eshop-subscribe` (id 3023074), products `eshop-pro` (id 7126957) and `basic-plan` (id 7126958), metered component `api-call` (id 3057195). This build does **not** seed them and does **not** verify them against a live site; treat the catalog as a given and configure the integration to consume it (resolve IDs from the stable handles).

**Phase 1 — Domain + provider seam.** In ApplicationCore: define the `Subscription` entity, `ISubscriptionService`, and the provider-agnostic `IBillingClient`. In Infrastructure: implement `MaxioBillingClient` (the single provider seam) behind that interface with a typed `HttpClient`, bind `MaxioSettings`, **resolve the base URL from configuration so the target server is switchable (prod/dev/local) per §2.3/§4.3**, and register everything in the composition roots. Expose a first read capability: list available plans.
...
**Phase 3 — UC2 Pay-as-you-go (priority).** The metered component (`api-call`, id 3057195, $0.01/unit) is already on family `eshop-subscribe` and therefore available on every `eshop-pro` / `basic-plan` subscription. Add the capability to record usage against a subscription's `api-call` component and to read the period-to-date total. Include the startup/first-call validation that the configured component handle resolves to a metered-kind component on the family. (Optionally wire the order-placed MediatR notification → one usage unit.)

**Phase 4 — UC3 / UC4 (plan change + lifecycle).** Preview + commit plan change with proration timing (between `eshop-pro` ↔ `basic-plan`); pause / resume / cancel (immediate or end-of-period) / reactivate; publish the corresponding state notifications on each transition. *Verify:* preview matches commit (or is rejected as stale); end-of-period cancel defers to the period boundary; notifications fire in-process.
...
- Production PCI hardening — demo uses test mode.
...
- **Live Maxio sandbox seeding, read-back, and verification** — the catalog is a pre-provisioned given (§1.3, UC0) and the build is validated against the readiness harness, not the live site. Do not seed, verify, or exercise the use cases against a live Maxio tenant.
- *Note:* making the target server configurable (§2.3) is **in** scope; standing up, invoking, investigating, or maintaining any endpoint the client targets is **not** — the implementer's only obligation is that `Maxio:BaseUrl` is honored when set.
...
- **Target server per environment:** the outbound base URL is configurable via `Maxio:BaseUrl` (§2.3/§5). Defaults: both hosts leave `Maxio:BaseUrl` empty in Development, so the subdomain-derived host is used unless a run profile overrides it. Each run profile supplies its value via `appsettings.{Environment}.json` or the `Maxio__BaseUrl` environment variable — never a code change. Invoking or investigating the target endpoint is not part of this plan's work; the deliverable is only that the knob exists and is honored.
- **Second plan for UC3:** `basic-plan` (id 7126958, $29/mo) alongside `eshop-pro` in family `eshop-subscribe`.
- **Metered component for UC2:** `api-call` (id 3057195) on family `eshop-subscribe`, type Metered, $0.01/unit. Component is on the family so it's automatically available to every subscription on either plan — no per-subscribe attach step needed.
- **Live testing:** omitted in this variant — no live-sandbox seeding, read-back, or use-case exercise against a live Maxio tenant. The readiness harness is the acceptance signal.
...
| **§2.3 (config conventions)** | **Primary statement of the requirement:** the `Maxio:BaseUrl` override, the explicit resolution order (override wins, else subdomain-derived), the prod/dev/local intent, the per-host/per-profile override note, and the caution that the client must not ignore the override. Clarified `Maxio:Environment` = region, not deployment target. |
...
| **§4.3 (wiring)** | **Mechanism:** the `AddHttpClient` snippet sets `BaseAddress` from `MaxioSettings.ResolveBaseUrl()` with the "override ?? derived" rule and a "do NOT hardcode the host" note. |
| §6 (implementation plan) | Implementing the use cases above is the main objective, plus the configurable target server. The target endpoint is a configuration target only — the implementer must not invoke or investigate it. |
| §7 (out of scope) | Clarified configurability is in scope; standing up, invoking, investigating, or maintaining an endpoint is not; live Maxio sandbox testing is out of scope. |
