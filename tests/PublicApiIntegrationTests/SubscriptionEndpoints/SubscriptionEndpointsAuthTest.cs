...
public class SubscriptionEndpointsAuthTest
{
    ...
    {
        var response = await ProgramTest.NewClient.PostAsync("api/subscriptions", new StringContent("{}", Encoding.UTF8, "application/json"));
        Assert.AreEqual(HttpStatusCode.Unauthorized, response.StatusCode);
    }
    ...
    [TestMethod]
    public async Task ListPlansAllowsAuthenticatedUser()
    {
