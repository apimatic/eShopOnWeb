...
public class TestApiApplication : WebApplicationFactory<AuthenticateEndpoint>
{
    ...
    protected override IHost CreateHost(IHostBuilder builder)
    {
        ...
        // Supply non-secret placeholder Maxio configuration so the host's fail-fast validation
        // passes in tests (these tests do not call the billing provider). Real credentials are
        // never committed; they are supplied via user-secrets/environment at runtime.
        builder.ConfigureAppConfiguration(config =>
        {
            config.AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["Maxio:ApiKey"] = "test-api-key-not-a-real-secret",
