...
| Customer linkage | `Customer`: `Id (id): string?` ← **set to OUR user id** · `MerchantCustomerId (merchant_customer_id): string?` (additional correlation field). Grounding: `Customer` summary = "a customer in **your** system"; the list operation's `customerId` param doc = "unique identifier representing a specific customer in **merchant's/partner's** system or records" (source-confirmed, `Api/Vault.cs`) — the SAME our-user-id value is used at creation and at listing. |
...
| Aspect | Contract |
|---|---|
| List signature | `ListCustomerPaymentTokens(string customerId, int? pageSize = 5, int? page = 1, bool? totalRequired = false, RequestOptions? requestOptions = null, CancellationToken ct = default)` — `customerId` = OUR user id (see Customer linkage above); wire params `customer_id`, `page_size`, `page`, `total_required` |
...
- **Assumption — vault customer id = our user id.** Grounded in source XML docs (`Api/Vault.cs`): `customer_id` and `Customer.Id` are merchant-system identifiers. We will set `Customer.Id` (and optionally `MerchantCustomerId`) to our user id at token creation and list with the same value.
